...
import originalConfig from "../../utils/config/timeline.json";
...
function createGraphic(node, PGA_val, PSP_val) {
  ...
  addDropout(
    g,
    config.width + node.getBoundingClientRect().width / 17,
    config.height / 2
  );
...
function addDropout(node, x, y) {
  ...
  g
    .append("text")
    .text("Avance en bachillerato")
    .attr("font-size", fontSize)
    .attr("font-weight", "bold");
  g
    .append("text")
    .text("Cursados: 14 / 28 (58%)")
    .attr("font-size", fontSize)
  ...
  g
    .append("text")
    .text(
      "Semestres estimados para terminar bachillerato: 3 (probabilidad 95%)"
    )
    .attr("font-size", fontSize)
