...
function createGraphic(node,programStructure,studentInfoAcademic){
  ...
  termsWidth = config.boxWidth * (config.terms)+config.paddingSubjectsBox*config.terms;
  termsScale = d3.scaleLinear().domain([1,config.terms + 1]).range([0,termsWidth]);
  ...
  graphicWidth = (config.boxWidth*config.expandibleFactorX) - (graphicPadding*2.5);
  graphicHeight = ((config.boxHeight*config.expandibleFactorY)-config.boxHeight)*0.30;
  histogramScaleX = d3.scaleLinear().domain([config.rangeGrades[0].minGrade,config.rangeGrades[config.rangeGrades.length-1].maxGrade]).range([0,graphicWidth]);
  histogramScaleY = d3.scaleLinear().range([graphicHeight,0]);
  histogramScaleY.domain(config.domainGradesHistogram);
  xAxis = d3.axisBottom().scale(histogramScaleX).ticks(7);
  yAxis = d3.axisLeft().scale(histogramScaleY).ticks(4);

 nestTerms = d3.nest()
               .key(function(d){return d.semester+'S'+d.year})
...
let y = parseInt(config.boxWidth*0.3);
  ...
  studentInfo.terms.forEach((term)=>{
    term.coursesTaken.forEach((course)=>{
      addStudentInfoToSubject(course,term.semester+'S'+term.year,term.semester,term.year);
    });
  });
  ...
  function addTermsButtons(nestTerms,yPosition){
    let terms = config.terms > nestTerms.length ? config.terms : nestTerms.length;
    let x = d3.scaleLinear().domain([0.8,terms+0.5]).range([0,terms*config.spacingBetweenTermsButtons]);
    let buttonWidth = parseInt(config.spacingBetweenTermsButtons/1.3);
    let buttons = svg.selectAll(".botonesTerm")
        .data(nestTerms).enter()
        .append("g")
        ...
        .attr("transform",(d,i)=>{
          ...
          return `translate(${x(i+1)+config.marginLeftTermsButtons-buttonWidth/2.5},${yPosition})`
        })
        .on("click",function(d,i){
            let ref = d3.select(this);
            svg.selectAll(".botonesTerm").select("rect")
                .attr("stroke",config.strokeColor)
                .attr("stroke-width",config.strokeWidth);

            d3.selectAll(".psp-circle").attr("fill",function(d,i){return d3.select(this).attr("originalColor")});

            d3.selectAll(".botonesTerm").filter(function(){
          ...
          if(ref.attr("clicked")=='false'){
            ...
            d3.select(this).attr("clicked","true").select("rect")
            ...
            d3.select("#PSP_"+i).attr("fill",function(d,i){return d3.select(this).attr("originalColor")});
            d3.select(this).attr("clicked","false").select("rect")
                .attr("stroke",config.strokeColor)
                .attr("stroke-width",config.strokeWidth);
        ...
        .attr("rx",parseInt(buttonWidth/8)).attr("ry",parseInt(buttonWidth/8))
        .attr("stroke-width",config.strokeWidth)
        .attr("stroke",config.strokeColor)
  ...
  function autoResize(node){
    ...
    let newspacingBetweenTermsButtons = parseInt(node.getBoundingClientRect().width / 27.81);
    let newboxHeight = parseInt(newboxWidth/(config.boxWidth/config.boxHeight));
    let newtextSubjectFontSize = parseInt((newboxWidth + newboxHeight)/((config.boxWidth+config.boxHeight)/config.textSubjectFontSize));
    let newpaddingSubjectsBox = parseInt((newboxWidth + newboxHeight)/((config.boxWidth+config.boxHeight)/config.paddingSubjectsBox));
    let newpaddingBetweenDegress = parseInt(newboxWidth/(config.boxWidth/config.paddingBetweenDegress));
    let newlabelDegreeTextFontSize = parseInt(newboxWidth/(config.boxWidth/config.labelDegreeTextFontSize));
    let newhistogramLabelFontSize = parseInt(newboxWidth/(config.boxWidth/config.histogramLabelFontSize));
    //let newpaddingBetweenTermsButtons = parseInt(newboxWidth/(config.boxWidth/config.paddingBetweenTermsButtons));
    let newtermsButtonsFontSize = parseInt(newboxWidth/(config.boxWidth/config.termsButtonsFontSize));
    let newstrokeWidthTermSelected = parseInt(newboxWidth/(config.boxWidth/config.strokeWidthTermSelected));
    let newtextSubjectSpacing= parseInt(newboxWidth/(config.boxWidth/config.textSubjectSpacing));
    let factor = parseInt(node.getBoundingClientRect().width / 2.27);
    let newmarginLeftTermsButtons = parseInt(factor/(1100/config.marginLeftTermsButtons));
...
function resizeSVG(){
    ...
    let maxY = d3.max(bottomSubjects.nodes(), function(d) {
      ...
      return (d3.select(d).attr("expanded")=='true')?yValue+parseInt(d3.select(d).select("rect").attr("height"))*config.expandibleFactorY:yValue+parseInt(d3.select(d).select("rect").attr("height"));
    });

    let maxX = d3.max(d3.selectAll(".term").nodes(), function(d) {
      return parseInt(d3.select(d).attr("x"))+config.boxWidth*config.expandibleFactorX;
    });
...
function unfocusSubjectsByTerm(){
  ...
  courseSVG.select("text.grade")
     .text((d)=>{ let lastDataTerm  = Object.keys(d)[Object.keys(d).length-1];
  ...
  let nonExpandedcourseSVG = d3.selectAll(`.subject.focusTerm[expanded='false']`);
  nonExpandedcourseSVG.select("rect")
            .attr("stroke",config.strokeColor)
            .attr("stroke-width",config.strokeWidth)
            .attr("fill",(d)=>{let lastDataTerm  = Object.keys(d)[Object.keys(d).length-1];
                  return getGradeColor(d[lastDataTerm].grade,d[lastDataTerm].state);
            })

  nonExpandedcourseSVG.select("path")
                      .attr("stroke",config.strokeColor);

  nonExpandedcourseSVG.select("circle.grade")
          .attr("visibility", "visible");

  nonExpandedcourseSVG.select("text.grade")
           .attr("visibility", "visible");
  ...
  ExpandedcourseSVG.select("path")
                   .attr("stroke",config.strokeColor);

  ExpandedcourseSVG.selectAll(".histogramCohort, .histogramGroup").attr("visibility","hidden")
                   .selectAll(".bar")
                   //.transition()
                   .attr("height",0);
  ExpandedcourseSVG.selectAll(".histogramCohort, .histogramGroup")
                    ...
                    .attr("height",function(d,i){return d3.select(this).attr("height-backup")});
...
function focusSubjectsByTerm(termId){
  ...
  let courseSVG = d3.selectAll(`.subject.Semester${termId}`);
    ...
    //Histograms
    let expandedcourseSVG = d3.selectAll(`.subject.Semester${termId}[expanded='true']`);
    expandedcourseSVG.selectAll(".histogramCohort, .histogramGroup").attr("visibility","hidden");
    expandedcourseSVG.selectAll(`.histogramCohort.Semester${termId}, .histogramGroup.Semester${termId}`)
             .attr("visibility","visible")
             .attr("transform","scale(1,1)")
             .selectAll(".bar")
             .transition()
  ...
  let nonExpandedcourseSVG = d3.selectAll(`.subject.Semester${termId}[expanded='false']`);
    nonExpandedcourseSVG.select("circle.grade")
            .attr("visibility", "visible");
    nonExpandedcourseSVG.select("text.grade")
              .attr("visibility", "visible");
...
function unfocus(subjects,boolean){
      if(boolean){
        ...
        subjects.select("rect").attr("fill-opacity",function(d,i){return (d)?0.2:0.8;});
        ...
        subjects.selectAll(".histogramGroup, .histogramCohort").attr("opacity",0.5);
      }else{
...
function showRequisites(course,boolean){
    ...
    function show(courseSVG,boolean){
      if(boolean){
        ...
        courseSVG.selectAll('.requirementText')
                 .attr("visibility",'visible')
                 .attr("transform",function(d,i){
                   if(courseSVG.attr("expanded")==='true'){
                     let x = d3.select(this).attr("x") ? d3.select(this).attr("x") : d3.select(this).attr("cx");
                     //console.log(x);
        ...
        courseSVG.select('rect')
                 ...
                 .attr("stroke-width",function(){
                   return courseSVG.attr("expanded")=='false' ? config.requirementStrokeWidth: config.requirementStrokeWidth/2;
                 })
                 .attr("fill",config.backgroundColor)
        ...
        courseSVG.select('rect').attr("stroke",config.strokeColor)
                  .attr("stroke-width",config.strokeWidth/2);
        ...
        courseSVG.select('rect')
                 .attr("fill",function(d,i){
                    return d3.select(this.parentNode).select('circle.grade').attr("fill");
                  })
                  .attr("stroke",config.strokeColor)
                  .attr("stroke-width",function(){
                    return courseSVG.attr("expanded")=='false' ? config.strokeWidth: config.strokeWidth/2;
                  })
...
function moveSubjectsOnY(node,boolean){
  ...
  let expandibleValue = config.boxHeight*config.expandibleFactorY - config.boxHeight;
  d3.select(node.node().parentNode).selectAll(".subject")
      .filter(function(){
...
function moveTermsOnX(node,boolean){
  ...
  let expandibleValue = config.boxWidth*config.expandibleFactorX - config.boxWidth;

  //Check if there are subjects in the term clicked, expanded.
  let expandedSubjects = d3.select(node.node().parentNode).selectAll('[expanded=true]');
  ...
  if(!((boolean && expandedSubjects.empty()) || (!boolean && expandedSubjects.size()===1))){return;}
  d3.select(node.node().parentNode.parentNode).selectAll(".term, .labelTerm")
...
function showHistogram(node,show){
  ...
  if(show){
      ...
      node.select("rect")
          .transition()
          .duration(config.transitionTiming)
          .attr("transform",`scale(${config.expandibleFactorX},${config.expandibleFactorY})`)
          .attr("stroke-width",config.strokeWidth/2)
          //.attr("fill",config.backgroundColor);
      node.select("path")
          .transition()
          .duration(config.transitionTiming)
          .attr("transform",`scale(${config.expandibleFactorX+0.2},${config.expandibleFactorY})`)

      node.selectAll(".gradeText,.reproved")
          .transition()
          .duration(config.transitionTiming)
         ...
         let histogramsGroup = node.selectAll(".histogramGroup");
           ...
           histogramsCohort.filter(function(d,i){
           ...
           histogramsGroup.filter(function(d,i){
      ...
      node.selectAll(".histogramCohort, .histogramGroup")
                            .transition()
                            .duration(config.transitionTiming)
...
function addTerm(node,xPosition,yPosition,term){
  ...
  term.courses.forEach((course,i)=>{
    addSubject(termSVG,0,newYPosition,config.boxWidth,config.boxHeight,course);
...
function addSubject(node,x,y,width,height,course){
  ...
  let g = node.append("g")
          ...
          .attr("expanded","false")
          .attr("req","false")
          .on("click",function(d,i){onclick(d3.select(this),course)})
          .on("mouseover",function(){d3.select(this).style("cursor", "pointer");})
          .on("mouseout",function(){d3.select(this).style("cursor", "default");});
  ...
  g.append("rect")
    .attr("width",width)
    .attr("height",height)
    .attr("rx",config.borderRadious).attr("ry",config.borderRadious)
    .attr("fill",config.backgroundColor)
  ...
  g.append("path")
    ...
    .attr("fill",config.backgroundColor)
    .attr("d",()=>{
      return `M${config.borderRadious},0 H${width - bandWidth} V${height} H${config.borderRadious} A${config.borderRadious},${config.borderRadious} 0 0 1 0,${height-config.borderRadious} V${config.borderRadious} A${config.borderRadious},${config.borderRadious} 0 0 1 ${config.borderRadious},0 Z`
    });
    ...
    wrapText(subjectText,config.textSubjectSpacing,width-bandWidth-config.textSubjectPadding*2,config.textSubjectPadding,config.textSubjectPadding,config.textSubjectFontSize,config.textSubjectFontWeight,course.name);

  // Grade label
  g.append("circle")
    .attr("class",'grade')
    .attr("fill-opacity",0)
    .attr("cx",width-gradeCircleSize).attr("cy",gradeCircleSize)
    .attr("fill",config.backgroundColor)
  ...
  g.append("circle")
    ...
    .attr("cx",width-(bandWidth*2)).attr("cy",height-gradeCircleSize*2)
  ...
  function onclick(nodeCourse,course){
    ...
    d3.selectAll(`.botonesTerm:not([id='${termSelected}'])`).select("rect")
    .attr("stroke",config.strokeColor)
    .attr("stroke-width",config.strokeWidth);
...
function addHistogram(node,y,className,term,distribution,grade,state,title){

  histogramScaleY.domain([1,d3.max(distribution)]);
  yAxis = d3.axisLeft().scale(histogramScaleY).ticks(4);
  if(title=='General')yAxis.tickFormat(function(d,i){return d+'%';});
  //console.log(distribution);
  let graphic = node.append("g").attr("transform",`translate(${graphicPadding},${y})`);
  ...
  g.append("text").attr("y",-barWidthXaxis*3.0).attr("x",0)
    .attr("font-size",config.histogramLabelFontSize)
    .attr("font-weight",'bold')
    .text(title);
  let xAxisSvg = g.append("g").call(xAxis);
  xAxisSvg.attr("transform",  `translate(${barWidthXaxis},${graphicHeight+barWidthXaxis})`);
  xAxisSvg.select("path").attr("visibility","hidden");
  xAxisSvg.selectAll(".tick").select("line").attr("visibility","hidden");
  xAxisSvg.append("rect").attr("fill","red").attr("width",50);

  g.append("g").call(yAxis).select("path").attr("visibility","hidden");

  g.selectAll(".tick").selectAll("text").attr("font-size",config.histogramLabelFontSize);
  ...
  g.selectAll(".bar")
   ...
   .append("rect")
   .classed("bar",true)
   .attr("x",(d,i)=>{return histogramScaleX(i+1)+barWidthXaxis})
   .attr("y",(d,i)=>{return histogramScaleY(d)})
   .attr("fill",(d,i)=>{
     if(i+1 == Math.floor(grade)){return config.backgroundColorBarsSelected;}
     else{return config.backgroundColorBars}
   })
   .attr("width",barWidth)
   .attr("height",0)
   .attr("height-backup",(d,i)=>{return (graphicHeight - histogramScaleY(d))< 0 ? 0:graphicHeight - histogramScaleY(d) });

   config.rangeGrades.forEach(function(d,i){
     let x1 = d.minGrade;
     let x2 = (i === config.rangeGrades.length - 1) ? d.maxGrade : config.rangeGrades[i+1].minGrade;
     xAxisSvg.append("rect").attr("fill",d.color).attr("width",histogramScaleX(x2)-histogramScaleX(x1)).attr("height",barWidthXaxis).attr("x",histogramScaleX(x1));
   });

   if(!config.showHistogramLine)return
   // Add lines
   let lineData = distribution.map(function(d,i){
     return {"x":histogramScaleX(i+1+0.5)+barWidthXaxis,"y":histogramScaleY(d)};
   });
  ...
  g.append("path")
   ...
   g.selectAll(".circle")
    ...
    .enter()
    .append("circle")
    .attr("cx",(d,i)=>{return histogramScaleX(i+1+0.5)+barWidthXaxis})
    .attr("cy",(d,i)=>{return histogramScaleY(d)})
...
function addStudentInfoToSubject(course,termId,semester,year){
  ...
  courseSVG.classed(`Semester${termId}`, true)
           .attr('term','');
  ...
  let distributionCohort = course.cohortGroup == null || course.cohortGroup.distribution == null ? [] :
      course.cohortGroup.distribution.map((element)=>{
          return element.value;
      });
  addHistogram(courseSVG,config.boxHeight*0.8,'histogramCohort',`Semester${termId}`,distributionCohort,grade,course.state,`Calificaciones ${semester} ${year}`);

  let distributionGroup = course.historicGroup == null || course.historicGroup.distribution == null ? [] :
      course.historicGroup.distribution.map((element)=>{
          return element.value;
      });

  addHistogram(courseSVG,config.boxHeight*2.05,'histogramGroup',`Semester${termId}`,distributionGroup,grade,course.state,'Calificaciones históricas');
}
...
function addReprovedCircles(){
  ...
  const padding = config.boxHeight*0.20;
  ...
  .append("g").classed("reproved",true)
  //.classed("hiddenElements",true)
  .selectAll("circle")
  .data(function(){
    let data = d3.select(this).data()[0];
    let array = Object.keys(data);array.pop();
    return array.map(function(d){return data[d]});
  }).enter()
    .append("circle").attr("r",gradeCircleSize)
...
function getGradeColor(grade,status){
  ...
  //Special situation
  if(grade == 0.0 && status === 'A') return config.rangeGrades[config.rangeGrades.length-1].color;
