...
class App extends Component{
    ...
    searchStudent(studentId){
      ...
      return new Promise((resolve,reject)=>{
        setTimeout(function(){
          ...
          component.getStudentacademics(studentId,component.props.programaId)
            .then((response)=>{
              let studentAcademic = response.data;
              let year = studentAcademic.planYear;
              let PSP = studentAcademic.terms.map(function(d,i){return d.termAvg});
              let PGA = studentAcademic.terms.map(function(d,i){return d.accAvg});
              component.setState({studentAcademic: studentAcademic,PSP:PSP,PGA:PGA});
              return component.getProgram(component.props.programaId,year);
            })
            .then((response)=>{
              let programStructure = response.data;
              let date = component.getDate(programStructure.lastDataUpdate);
              component.setState({programStructure: programStructure,lastDate:date});
    ...
    render(){
      return (
        <div>
            <Nav searchFunction = {this.searchStudent} programa={this.props.programa} lastDate={this.state.lastDate}></Nav>
            {
              this.state.programStructure && this.state.studentAcademic &&
              <div>
                <D3Component PGA={this.state.PGA} PSP={this.state.PSP}></D3Component>
                <D3DashboardComponent Program={this.state.programStructure} StudentAcademic={this.state.studentAcademic}></D3DashboardComponent>
              </div>
            }

            <Toast onRef={ref => (this.toast = ref)}></Toast>
        </div>
      );
    }
}
...
App.defaultProps = {
  programa: dataServer.user.programa.name,
  programaId: dataServer.user.programa.id
};
